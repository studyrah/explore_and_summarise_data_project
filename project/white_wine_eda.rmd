...
```{r echo=FALSE, message=FALSE, warning=FALSE, Plot_One, fig.height=6}
# save typing errors for plot labels
alcohol_lab <- "alcohol (% by volume)"

# freqpoly - palmed off to a function to save repetition
freqpolyByHMLCategory(wines, 'alcohol', 'quality.category') +
    theme(legend.position="right") + ylab("% wines") + xlab(alcohol_lab) + 
    ggtitle("Alcohol Breakdown by Quality")
```
...
The density plot shows quite clearly the marked increase in quality as you 
increasethe level of alcohol, with a sweetspot between around 11 to 13.

The distribution of low and medium quality wines is 'quite similar' whereas 
(as expected) the distribution of high quality wines is noticeably right shifted 
(higher alcohol).
...
ggplot(aes(x = chlorides, 
  ...
  ggtitle("Chlorides and Alcohol Combined Effect on Quality")
```

Here we plot chlorides against alcohol facet wrapped by qaulity to see whether 
they re-inforce each other.  In particular to see whether the high quality 
clusters (large and small) outlined earlier still exist when we combine features.

Although it is difficult to be certain, particularly because of the uneven 
distribution (there are many more medium quality wines), it looks like the two 
high quality clusters hypothesis just about still holds.
...
Moreover (with a bit of a squint) it looks like the critical mass of low/medium 
quality wines sit somewhere between the two clusters.
...
difference in distribution for high quality wines compared to the rest.  
We see a fairly strong positive correlation (Pearson 0.5) for high quality wines
vs virtually nothing for the rest.

So, getting the balance of volatile.acidity to alcohol level correct might be
a further worthwhile consideration when producting/predicting/evaluating white
wine.
